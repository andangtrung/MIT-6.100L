...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        filtered_list.append(tuple(int(i) for i in matrix_multiply(filter_matrix, pixel)))
...
def reveal_bw_image(filename):
    ...
    revealed_pixels_list = []
    for pixel in pixels_list:
        revealed_pixels_list.append(extract_end_bits(1, pixel) * 255)
    img = Image.open(filename)
    return pix_to_img(revealed_pixels_list, img.size, img.mode)
...
def reveal_color_image(filename):
    ...
    revealed_pixels_list = []
    for pixel in pixels_list:
        revealed_pixels_list.append(tuple(int(extract_end_bits(3, i) * 255 / 7) for i in pixel))
    img = Image.open(filename)
    return pix_to_img(revealed_pixels_list, img.size, img.mode)
