...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    diff = 0
    all = 0
    for key, freq1 in freq_dict1.items():
        freq2 = freq_dict2.get(key, 0)
        diff += abs(freq1 - freq2)
        all += freq1 + freq2
    for key, freq2 in freq_dict2.items():
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    """
    words_list = text_to_list(load_file(file_path))
    freq_dict = get_frequencies(words_list)
    tf = {}
    for key in freq_dict.keys():
        tf[key] = freq_dict[key] / len(words_list)
    return tf
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for file_path in file_paths:
        words_list = text_to_list(load_file(file_path))
        file_freq = get_frequencies(words_list)
        for word in file_freq.keys():
            freq_dict[word] = freq_dict.get(word, 0) + 1
    idf = {}
    for word, freq in freq_dict.items():
        idf[word] = math.log10(len(file_paths) / freq)
    return idf
